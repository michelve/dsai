...
- Prefer importing from `@dsai/react/types` for type-only usage, or `@dsai/react` for public API exposure.
- Runtime responsive helpers should be imported from `@dsai/react/utils/responsive` to keep the types barrel type-only:
  - `import { getResponsiveValue, isResponsiveValue } from '@dsai/react/utils/responsive';`
...
- Type assertions live in `packages/@dsai/react/src/types/types.test.ts` and include negative cases (`@ts-expect-error`) to prevent regressions. Extend these when adopting polymorphic or responsive patterns in components.
