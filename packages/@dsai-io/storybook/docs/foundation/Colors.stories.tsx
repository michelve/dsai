...
const COLOR_BS_DARK = 'var(--bs-dark)';
const COLOR_BS_WHITE = 'var(--bs-white)';
        ...
        <div style={{ display: 'flex', gap: SPACING_3, flexWrap: 'wrap', marginTop: '24px' }}>
          {brandColors.map((color) => (
            <ColorSwatch
              ...
              textColor={Number.parseInt(color.name, 10) >= 500 ? COLOR_BS_WHITE : COLOR_BS_DARK}
            />
          ))}
        </div>
      </div>
...
export const SemanticColors: Story = {
  render: () => {
    const semanticColors = [
      { name: 'Primary', key: 'primary', description: 'Main brand color', textColor: COLOR_BS_WHITE },
      { name: 'Secondary', key: 'secondary', description: 'Secondary actions', textColor: COLOR_BS_DARK },
      { name: 'Success', key: 'success', description: 'Success states', textColor: COLOR_BS_WHITE },
      { name: 'Danger', key: 'danger', description: 'Error states', textColor: COLOR_BS_WHITE },
      { name: 'Warning', key: 'warning', description: 'Warning states', textColor: COLOR_BS_DARK },
      { name: 'Info', key: 'info', description: 'Informational', textColor: COLOR_BS_WHITE },
    ].map(({ key, ...rest }) => {
...
export const ComponentSemanticColors: Story = {
  render: () => {
    ...
    const sem = (name: string, key: string, description: string, textColor = COLOR_BS_DARK) => {
    ...
    const componentSemanticColors = [
      {
        category: 'Body & Text',
        colors: [
          sem('Body Text', 'body-color', 'Default body text color', COLOR_BS_WHITE),
          sem('Body Background', 'body-bg', 'Default body background'),
          sem('Emphasis', 'emphasis-color', 'High contrast text', COLOR_BS_WHITE),
      ...
      },
      {
        category: 'Links',
        colors: [
          sem('Link Default', 'link-color', 'Default link color', COLOR_BS_WHITE),
          sem('Link Hover', 'link-hover-color', 'Link hover state', COLOR_BS_WHITE),
        ],
      },
      {
      ...
      },
      {
        ...
        colors: [
          ...
          sem('Border Translucent', 'border-color-translucent', 'Translucent borders', COLOR_BS_WHITE),
        ],
      },
      ...['Primary', 'Success', 'Danger', 'Warning', 'Info'].map((variant) => {
        ...
        return {
          ...
          colors: [
            sem('Text Emphasis', `${prefix}-text-emphasis`, `${variant} text emphasis`, COLOR_BS_WHITE),
                ...
                {steps.map((step) => {
                  ...
                  return (
                    <ColorSwatch
                      ...
                      cssVariable={cssVar}
                      tokenPath={tokenPath}
                      textColor={stepNum >= 500 ? COLOR_BS_WHITE : COLOR_BS_DARK}
                    />
                  );
                })}
              </div>
