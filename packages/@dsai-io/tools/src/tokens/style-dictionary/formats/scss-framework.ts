 ...
 * ```
 */
function formatScssToken(
  token: { name: string; $value?: unknown; value?: unknown; $description?: string; description?: string },
  framework: FrameworkTarget,
  customMappings: Record<string, string>,
  variablePrefix: string,
  lines: string[]
...
export const scssFrameworkVariables: FormatDefinition = {
  ...
  format: ({ dictionary, options }: SDFormatArgs): string => {
    ...
    for (const token of dictionary.allTokens) {
      formatScssToken(token, framework, customMappings, variablePrefix, lines);
    }
