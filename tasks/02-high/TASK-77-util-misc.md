# Task: Audit Utility – `misc` suite

**Task ID:** TASK-77
**Title:** Audit and harden miscellaneous utilities in `packages/@dsai/react/src/utils/misc`
**Priority:** High
...
- **Name:** Misc utilities (`normalizeTriggers`, `getSafeInputProps`, `clearAllEvent`, `mapPlacement`, `toggleAllEvent`, `toggleItemEvent`, `selectAllEvent`)
- **File:** `packages/@dsai/react/src/utils/misc/*`
- **Category:** misc / interaction helpers
- **TypeScript:** Yes (strict)
- **Used By (key components/hooks):**
  - Exported via `packages/@dsai/react/src/utils/misc/index.ts` and re-exported from `packages/@dsai/react/src/index.ts`
...
- [ ] Pending review per template (event safety, SSR handling where relevant).

### 2.4 Accessibility (If Applicable)

- [ ] Pending review (ARIA/keyboard/mouse implications if applicable).
...
- Pending comparison against similar interaction helpers provided by reference systems.
...
## 6. Definition of Done (DoD) for `misc` utilities
