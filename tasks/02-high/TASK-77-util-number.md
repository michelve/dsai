# Task: Audit Utility – `number` suite
...
- **Name:** Number utilities (`clamp`, `formatNumber`, `formatCurrency`)
- **File:** `packages/@dsai/react/src/utils/number/*`
- **Category:** number / formatting
- **TypeScript:** Yes (strict)
- **Used By (key components/hooks):**
  - Exported via `packages/@dsai/react/src/utils/number/index.ts` and re-exported from `packages/@dsai/react/src/index.ts`
...
- [ ] Pending review per template (input validation, locale/Intl behavior, error handling).
...
- Pending comparison against number/formatting utilities provided by reference systems.
...
## 6. Definition of Done (DoD) for `number` utilities
