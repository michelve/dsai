# Task: Audit Utility – `date` suite

**Task ID:** TASK-77
**Title:** Audit and harden date utilities in `packages/@dsai/react/src/utils/date`
**Priority:** High
...
- **Name:** Date utilities (`formatDate`, `formatRelativeTime`)
- **File:** `packages/@dsai/react/src/utils/date/*`
- **Category:** date/time formatting
- **TypeScript:** Yes (strict)
- **Used By (key components/hooks):**
  - Exported via `packages/@dsai/react/src/utils/date/index.ts` and re-exported from `packages/@dsai/react/src/index.ts`
...
- [ ] Pending review per template (locale handling, time zone assumptions, error handling).
...
- Pending comparison against date/formatting helpers provided by reference systems.
...
## 6. Definition of Done (DoD) for `date` utilities
