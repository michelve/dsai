# Task: Audit Utility – `string` suite
...
- **Name:** String utilities (`truncate`, `getVariantClass`, `slugify`, `capitalize`)
- **File:** `packages/@dsai/react/src/utils/string/*`
- **Category:** string / formatting
- **TypeScript:** Yes (strict)
- **Used By (key components/hooks):**
  - Exported via `packages/@dsai/react/src/utils/string/index.ts` and re-exported from `packages/@dsai/react/src/index.ts`
...
- [ ] Pending review per template (input validation, localization, error handling).
...
- Pending comparison against string helpers provided by reference systems.
...
## 6. Definition of Done (DoD) for `string` utilities
