# Task: Audit Utility – `types` suite

**Task ID:** TASK-77
**Title:** Audit and harden type utilities in `packages/@dsai/react/src/utils/types`
**Priority:** High
...
- **Name:** Type utilities (`isExternalUrl`, shared type helpers in `shared.ts`)
- **File:** `packages/@dsai/react/src/utils/types/*`
- **Category:** types / guards
- **TypeScript:** Yes (strict)
- **Used By (key components/hooks):**
  - Exported via `packages/@dsai/react/src/utils/types/index.ts` and re-exported from `packages/@dsai/react/src/index.ts`
...
- [ ] Pending review per template (runtime guards, error handling).
...
- Pending comparison against type/guard helpers provided by reference systems.
...
## 6. Definition of Done (DoD) for `types` utilities
