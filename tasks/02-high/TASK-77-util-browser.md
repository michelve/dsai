# Task: Audit Utility – `browser` suite
...
- **Name:** Browser utilities (`isBrowser`, `prefersReducedMotion`)
- **File:** `packages/@dsai/react/src/utils/browser/*`
- **Category:** browser / environment detection
- **TypeScript:** Yes (strict)
- **Used By (key components/hooks):**
  - Exported via `packages/@dsai/react/src/utils/browser/index.ts` and re-exported from `packages/@dsai/react/src/index.ts`
...
- [ ] Pending review per template (SSR guards, null checks, error handling).

### 2.4 Accessibility (If Applicable)

- [ ] N/A or pending review (only if accessibility impacts are detected).
...
- Pending comparison; align with detection helpers or hooks offered by reference design systems.
...
## 6. Definition of Done (DoD) for `browser` utilities
