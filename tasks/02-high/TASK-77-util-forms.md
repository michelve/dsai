# Task: Audit Utility – `forms` suite
...
- **Name:** Form utilities (`validateField`, `isFormDirty`, `getFieldError`, `createValidator`, `serializeForm`, `validateForm`, `submitForm`, `parseFormData`, `resetForm`, `isFormValid`)
- **File:** `packages/@dsai/react/src/utils/forms/*`
- **Category:** forms / validation
- **TypeScript:** Yes (strict)
- **Used By (key components/hooks):**
  - Exported via `packages/@dsai/react/src/utils/forms/index.ts` and re-exported from `packages/@dsai/react/src/index.ts`
...
- [ ] Pending review per template (input validation, form data safety, SSR considerations).

### 2.4 Accessibility (If Applicable)

- [ ] Pending review (error messaging, ARIA implications, keyboard/focus interactions if any helpers apply).
...
- Pending comparison against form helpers provided by reference systems.
...
## 6. Definition of Done (DoD) for `forms` utilities
