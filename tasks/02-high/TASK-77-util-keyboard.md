# Task: Audit Utility – `keyboard` suite
...
- **Name:** Keyboard utilities (`isEnterKey`, `isEscapeKey`)
- **File:** `packages/@dsai/react/src/utils/keyboard/*`
- **Category:** keyboard / input handling
- **TypeScript:** Yes (strict)
- **Used By (key components/hooks):**
  - Exported via `packages/@dsai/react/src/utils/keyboard/index.ts` and re-exported from `packages/@dsai/react/src/index.ts`
...
- [ ] Pending review per template (event typing, browser guards, error handling).

### 2.4 Accessibility (If Applicable)

- [ ] Pending review (keyboard semantics, key matching reliability).
...
- Pending comparison against keyboard helpers provided by reference systems.
...
## 6. Definition of Done (DoD) for `keyboard` utilities
