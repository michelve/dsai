...
async function run() {
  ...
  const existingDevResources = (await getDevResources()).reduce((into, curr) => {
...
async function deleteDevResources(devResourceIds) {
  ...
  async function loop() {
    ...
    console.log(`Deleting ${ids.length} out of ${remaining} (${totalSize} total)`);
...
async function deleteDevResource(id) {
  const fileResponse = await fetch(`${URL_BASE}/${FILE_KEY}/dev_resources/${id}`, {
    method: 'DELETE',
